...
import type { AggregationOptions, SessionAggregation } from '../core/reporting/timeBuckets';
...
type NormalizedOptions = Required<Pick<AggregationOptions, 'includeArchived' | 'includeCompleted'>>;

const normalizeOptions = (options: AggregationOptions = {}): NormalizedOptions => ({
...
const storeCache = new WeakMap<TaskStoreReadable, Map<string, Readable<ReportingSelectorResult>>>();
...
export const createTimeAggregationSelector = (
	...
	const selector = derived(storeRef, ($state) => {
		...
		const aggregated = aggregateProjects(ensureProjects($state?.data), normalizedRange, normalizedOptions);
