...
} from "$lib/core/normalization";
import {
  createTaskDataExport,
  parseTaskDataFile,
  revokeTaskDataExport,
} from "$lib/core/exporter";
...
export type SessionMutationResult =
  | { ok: true }
  | { ok: false; reason: SessionMutationFailureReason };
...
const loadInitialData = (): TaskData => {
  ...
  try {
    ...
    console.warn(
      "Failed to parse stored task data, resetting to defaults",
      error,
    );
    return fallback;
  }
  ...
  tickHandle = window.setInterval(() => {
    measureAndRecord(() =>
      store.update((state) => {
        ...
        const { projects, changed } = incrementTaskTime(
          state.data.projects,
          activeId,
          delta,
        );
...
export const taskStore = {
  ...
  setActiveProject(projectId: string) {
    withDataUpdate((data) => {
      ...
      const locatedSelection = data.selectedTaskId
        ? locateTask(data.projects, data.selectedTaskId)
        : null;
      const selectedTaskId =
        locatedSelection && locatedSelection.project.id === projectId
          ? locatedSelection.task.id
          : null;
  ...
  deleteProject(projectId: string) {
    ...
    store.update((state) => {
      ...
      const activeTaskStillPresent =
        state.data.activeTaskId && locateTask(projects, state.data.activeTaskId)
          ? state.data.activeTaskId
          : null;

      const activeTaskId = activeTaskStillPresent ?? null;
      clearedTimer = Boolean(
        state.data.activeTaskId && !activeTaskStillPresent,
      );

      const selectedTaskId =
        state.data.selectedTaskId &&
        locateTask(projects, state.data.selectedTaskId)
          ? state.data.selectedTaskId
          : null;
  ...
  createSiblingTaskAfter(taskId: string) {
    ...
    store.update((state) => {
      const { projects, task, changed } = addTaskAfter(
        state.data.projects,
        taskId,
        "",
      );
  ...
  deleteTask(taskId: string) {
    ...
    store.update((state) => {
      ...
      const baseData: TaskData = {
        ...
        selectedTaskId:
          state.data.selectedTaskId === taskId
            ? null
            : state.data.selectedTaskId,
      ...
      return {
        ...
        data: nextData,
        previewTaskId:
          state.previewTaskId === taskId ? null : state.previewTaskId,
        timerStartedAt: wasActive ? null : state.timerStartedAt,
        lastTickAt: wasActive ? null : state.lastTickAt,
        focusedEditorTaskId:
          state.focusedEditorTaskId === taskId
            ? null
            : state.focusedEditorTaskId,
      };
    });
  ...
  updateSession(
    ...
    withDataUpdate((data) => {
      ...
      const targetSession = located.task.sessions.find(
      ...
      const startIso = normalizeSessionTimestamp(
        updates.startedAt ?? targetSession.startedAt,
      );
      if (!startIso) {
      ...
      const endedAtSource = Object.prototype.hasOwnProperty.call(
        updates,
        "endedAt",
      )
        ? updates.endedAt
      ...
      const { projects, changed } = updateTaskSessionInTree(
        data.projects,
        taskId,
        sessionId,
        sessionPayload,
  ...
  deleteSession(taskId: string, sessionId: string): SessionMutationResult {
    ...
    withDataUpdate((data) => {
      ...
      const hasSession = located.task.sessions.some(
        (session) => session.id === sessionId,
      );
      if (!hasSession) {
        outcome = { ok: false, reason: "session-missing" };
      ...
      const { projects, changed } = deleteTaskSessionInTree(
        data.projects,
        taskId,
        sessionId,
      );
  ...
  startTask(taskId: string) {
    ...
    if (
      currentState.data.activeTaskId === taskId &&
    ...
    store.update((state) => {
      ...
      const recentTaskIds = [
        taskId,
        ...state.data.recentTaskIds.filter((id) => id !== taskId),
      ].slice(0, 5);
  ...
  completeTask(taskId: string) {
    let clearedTimer = false;
    store.update((state) => {
      const { projects, changed } = setTaskStatus(
        state.data.projects,
        taskId,
        "completed",
      );
      ...
      const activeTaskId =
        state.data.activeTaskId === taskId ? null : state.data.activeTaskId;
  ...
  selectTask(taskId: string | null) {
    store.update((state) => {
      ...
      const nextSelected =
        requestedId && locateTask(state.data.projects, requestedId)
          ? requestedId
          : null;
