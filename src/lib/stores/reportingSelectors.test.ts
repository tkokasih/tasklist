...
const createSession = (start: string, end: string | null, durationMs: number): TaskSession => ({
	id: `session-${start}`,
	startedAt: start,
...
const getResult = (selector: ReturnType<typeof createTimeAggregationSelector>): ReportingSelectorResult => get(selector);
...
describe('reportingSelectors', () => {
	...
	it('aggregates project tasks into totals map', () => {
		const child = makeTask({
			...
			title: 'Child',
			status: 'in-progress',
			sessions: [createSession(iso('2024-01-03', '10:00:00'), iso('2024-01-03', '10:30:00'), 30 * 60 * 1000)],
		...
		const parent = makeTask({
			...
			title: 'Parent',
			status: 'completed',
			sessions: [createSession(iso('2024-01-03', '09:00:00'), iso('2024-01-03', '10:00:00'), 60 * 60 * 1000)],
			timeSpentMs: 90 * 60 * 1000,
			children: [child]
		});

		const store = writable({ data: withProjects([makeProject('A', [parent])]) });
	...
	it('memoizes selectors per store/range/options combination', () => {
		...
		const selectorA = createTimeAggregationSelector(rangeToday, {}, store, true);
		const selectorB = createTimeAggregationSelector(rangeToday, {}, store, true);
	...
	it('updates when store projects change', () => {
		const project = makeProject('A', [
			makeTask({
				...
				title: 'Task 1',
				status: 'completed',
				sessions: [createSession(iso('2024-01-03', '08:00:00'), iso('2024-01-03', '09:00:00'), 60 * 60 * 1000)],
		...
		expect(getResult(selector).totals.get('task-1')?.totalMs).toBe(60 * 60 * 1000);
		...
		const updatedProject = makeProject('A', [
			makeTask({
				...
				title: 'Task 1',
				status: 'completed',
				sessions: [createSession(iso('2024-01-03', '08:00:00'), iso('2024-01-03', '10:00:00'), 2 * 60 * 60 * 1000)],
				timeSpentMs: 2 * 60 * 60 * 1000
			})
		]);

		store.set({ data: withProjects([updatedProject]) });
		expect(getResult(selector).totals.get('task-1')?.totalMs).toBe(2 * 60 * 60 * 1000);
	});
	...
	it('respects includeArchived option when aggregating', () => {
		const archivedTask = makeTask({
			...
			title: 'Archived',
			status: 'archived',
			sessions: [createSession(iso('2024-01-03', '07:00:00'), iso('2024-01-03', '08:00:00'), 60 * 60 * 1000)],
			timeSpentMs: 60 * 60 * 1000
		});

		const store = writable({ data: withProjects([makeProject('A', [archivedTask])]) });
		const defaultSelector = createTimeAggregationSelector(rangeToday, {}, store, true);
		expect(getResult(defaultSelector).totals.get('archived-task')).toBeUndefined();

		const includeArchivedSelector = createTimeAggregationSelector(rangeToday, { includeArchived: true }, store, true);
	...
	it('surfaces active overlap warnings when open sessions exist', () => {
		const task = makeTask({
			...
			title: 'Open session',
			status: 'in-progress',
			sessions: [createSession(iso('2024-01-03', '11:00:00'), null, 60 * 60 * 1000)],
			timeSpentMs: 60 * 60 * 1000
		});
	...
	it('returns empty totals when feature flag disabled', () => {
		...
		const store = writable({ data: withProjects([makeProject('A', [task])]) });
		const selector = createTimeAggregationSelector(rangeToday, {}, store, false);
		const result = getResult(selector);
