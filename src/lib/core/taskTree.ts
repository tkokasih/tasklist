 ...
 */
export const DEFAULT_STATUS_FILTERS: TaskStatus[] = [
  "idle",
  "in-progress",
  "paused",
];
...
export const updateTaskById = (
  ...
  const nextProjects = projects.map((project) => {
    const { next, changed: projectChanged } = mapTasks(
      project.tasks,
      project.id,
      updater,
      taskId,
...
const locateTaskRecursive = (
  ...
  for (let index = 0; index < tasks.length; index += 1) {
    ...
    const located = locateTaskRecursive(task.children, taskId, project, [
      ...parentIds,
      task.id,
    ]);
    if (located) {
...
const reorderTasksRecursive = (
  ...
  for (let i = 0; i < tasks.length; i += 1) {
    const task = tasks[i];
    const { next: childTasks, changed } = reorderTasksRecursive(
      task.children,
      targetId,
      delta,
    );
...
export const moveTask = (
  ...
  let changed = false;
  const nextProjects = projects.map((project) => {
    const { next, changed: projectChanged } = reorderTasksRecursive(
      project.tasks,
      taskId,
      delta,
    );
...
const appendTaskRecursive = (
  ...
  const next = tasks.map((task) => {
    ...
    const { next: childTasks, changed: childChanged } = appendTaskRecursive(
      task.children,
      parentId,
      newTask,
    );
...
export const addTaskAfter = (
  ...
  const project = nextProjects.find(
    (candidate) => candidate.id === located.project.id,
  );
  if (!project) {
    return { projects, task: null, changed: false };
  }
...
const cloneSessions = (sessions: TaskSession[]): TaskSession[] =>
    ...
    ...(() => {
      ...
      if (deltaMs > 0) {
        if (
          ...
          nextSessions[nextSessions.length - 1].endedAt
        ) {
        ...
        for (
          let index = nextSessions.length - 1;
          index >= 0 && remaining > 0;
          index -= 1
        ) {
 ...
 */
export const applySnapshot = (snapshot: TaskSnapshot): Project[] =>
  cloneProjects(snapshot.data);
