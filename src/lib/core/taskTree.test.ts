...
const projectWithTasks = (): Project => {
	...
	return createProject('Demo Project', {
		...
		createdAt: baseTime,
		updatedAt: baseTime,
		tasks: [root, createTask('Second task', { id: 'task-2', createdAt: baseTime, updatedAt: baseTime })]
	});
};
...
describe('taskTree core helpers', () => {
	...
	it('adds new tasks at root and as children', () => {
		...
		const childAdded = addTask(rootAdded, 'project-1', 'Child sibling', 'task-root');
	...
	it('updates task status and archived timestamp correctly', () => {
		...
		const resumed = setTaskStatus(archived.projects, 'task-root', 'in-progress');
	...
	it('flattens projects into located task list', () => {
		...
		expect(flattened.map((item) => item.task.id)).toEqual(['task-root', 'task-child', 'task-2']);
