<script lang="ts">
  ...
  import {
    ...
    isTimerRunning,
    recentTasks,
  } from "$lib/stores/taskSelectors";
  import CollapsibleCard from "./CollapsibleCard.svelte";

  $: current = $activeTask;
  $: running = $isTimerRunning;
  $: recents = $recentTasks;
  $: currentSession = current?.sessions?.length
    ? current.sessions[current.sessions.length - 1]
    : null;
  $: currentSessionElapsed =
    currentSession && !currentSession.endedAt
      ? Math.max(0, currentSession.durationMs)
      : 0;
...
<CollapsibleCard
  title="Active tasks"
  subtitle="Control playback and jump to recent work"
>
  <div class="space-y-4">
    <div
      class="flex flex-col items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-center text-slate-700"
    >
      ...
      {#if current}
        ...
        <div class="flex flex-col items-center gap-1 text-sm text-slate-600">
          <p class="font-mono text-3xl font-semibold text-blue-700">
            {formatDuration(current.timeSpentMs)}
          </p>
          <p class="font-medium">Total tracked time</p>
          {#if running && currentSession}
            <p class="text-xs tracking-wide text-blue-600 uppercase">
      ...
      {:else}
        <p class="text-sm text-slate-500">
          No active task. Select one from the list to get started.
        </p>
        <button
            ...
            >
              <div class="flex flex-col">
                ...
                <p class="text-xs text-slate-500">
                  Tracked {formatDuration(task.timeSpentMs)}
                </p>
              </div>
              <button
