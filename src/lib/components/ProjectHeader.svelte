...
<section class="space-y-4 rounded-2xl border border-slate-200 bg-white/70 p-6 shadow-md">
	<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
		<div class="flex flex-col gap-2">
			{#if project}
				{#if editing}
					<input
						class="max-w-xl rounded border border-blue-300 px-3 py-2 text-2xl font-semibold text-slate-800 shadow-inner focus:outline-none focus:ring-2 focus:ring-blue-400"
				...
				<p class="text-sm text-slate-500">
					Last saved {state.data.lastSavedAt ? `on ${formatTimestamp(state.data.lastSavedAt)}` : 'recently'}
				</p>
			{:else}
		...
		<div class="flex flex-wrap items-center gap-3">
			<select
				class="rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200"
				bind:value={state.data.activeProjectId}
				on:change={(event) => taskStore.setActiveProject((event.target as HTMLSelectElement).value)}
			>
			...
			{#if creating}
				...
				>
					<input
						class="w-40 rounded border border-blue-300 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
						placeholder="Project name"
						bind:value={newProjectTitle}
						bind:this={createInput}
					/>
					<button class="rounded border border-blue-500 px-2 py-1 text-xs font-semibold text-blue-600" type="submit">
						Create
					</button>
					<button
