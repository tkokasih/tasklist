...
<CollapsibleCard title="Data" subtitle="Manage snapshots and backups">
	<div class="space-y-4 text-sm text-slate-600">
		<div class="flex flex-col gap-2">
			<label class="text-xs font-semibold uppercase tracking-wide text-slate-500" for="snapshot-name">Save snapshot</label>
			<div class="flex flex-col gap-2 md:flex-row">
				<input
					class="flex-1 rounded border border-slate-300 px-3 py-2 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200"
		...
		<div class="flex flex-col gap-3">
			<p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Import or export data</p>
			<div class="flex flex-wrap gap-3">
				...
				<label class="flex cursor-pointer items-center gap-2 rounded border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
		...
		{#if state.data.snapshots.length > 0}
			<div class="space-y-2">
				<p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Snapshots</p>
				<ul class="space-y-3">
					{#each state.data.snapshots as snapshot (snapshot.id)}
						<li class="flex flex-col gap-2 rounded border border-slate-200 bg-slate-50 p-3 md:flex-row md:items-center md:justify-between">
							<div>
								<p class="font-medium text-slate-700">{snapshot.name}</p>
								<p class="text-xs text-slate-500">Saved {formatTimestamp(snapshot.createdAt)}</p>
							</div>
		...
		{:else}
			<p class="text-xs text-slate-500">No snapshots yet. Save one to capture the current state of your projects.</p>
		{/if}
