...
function drawIcon(canvas) {
	...
	drawGrid(canvas, s(96), s(128), s(320), s(320), 4, rgba('#0b1220', 0.4), stroke(2));
	...
	drawRoundedRect(canvas, s(170), s(360), s(210), s(60), s(24), COLORS.aquamarine);
...
function setPixel(canvas, x, y, color) {
	...
	canvas.data[index] = Math.round(color.r * blend + canvas.data[index] * (1 - blend));
	canvas.data[index + 1] = Math.round(color.g * blend + canvas.data[index + 1] * (1 - blend));
	canvas.data[index + 2] = Math.round(color.b * blend + canvas.data[index + 2] * (1 - blend));
